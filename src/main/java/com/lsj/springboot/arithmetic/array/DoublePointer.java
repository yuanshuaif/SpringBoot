...
public class DoublePointer {
     ...
     * 输入: nums1 = [1,2,3,6,0,0,0], m = 4   nums2 = [2,5,6],       n = 3    输出: [1,2,2,3,5,6,6]
     * @param nums1
     * @param m
     * @param nums2
     * @param n
    ...
    public static Integer[] combine(Integer[] firstList, Integer[] secondList){
        ...
        int i = 0, j = 0;
        for(; i < firstList.length && j < secondList.length;){
            if(firstList[i] == secondList[j]){
                combine.add(firstList[i++]);
                combine.add(secondList[i++]);
        ...
        if(j < firstList.length){
            while (j < firstList.length){
                combine.add(firstList[j++]);
            }
        }
